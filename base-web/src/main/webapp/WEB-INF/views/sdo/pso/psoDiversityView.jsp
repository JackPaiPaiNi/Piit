					...
					<table cellspacing="0" cellpadding="0">
						...
						<tr>
							<td>
								<b>是否白电：</b>
								<c:choose>
								   <c:when test="${PsoVo.sfBd == '0'}">否
								   </c:when>
								   <c:when test="${PsoVo.sfBd == '1'}">是 
								   </c:when>
								</c:choose></td>
...
<script type="text/javascript">
	$(function($) {
		...
		$('#btn-back').click(function() {
			//location.href = "<c:url value='/pso/diversityPso'/>";
			window.history.back(-1);
		});
		//打印
		$("#btn-print").click(function(){
			window.open("<c:url value='/pso/diversityPso/printPage'/>?id=" + $("input[name='id']").val());
		})
	});
	//打开订单查看界面
