			...
			<div class="col-xs-12">
				...
				<form id="form-submit">
					...
					<div class="tab-content">
					    ...
					    <div class="space-4"></div>
					    <c:import url="/pub/showLog/spoPso" charEncoding="UTF-8" />
					</div>
				</form>
				<!-- PAGE CONTENT ENDS -->
			</div>
...
<script type="text/javascript">
	...
	function initItmeGrid(){
		$(grid_selector_item).bindTable({
			...
			colModel : [ {name : 'id',label : 'ID',width : 60, hidden : true, editable : false,frozen:true} ,
					     ...
					     {name : 'bz',index : 'bz',label : '币种',editable:false, width:60},
						 ...
						 {name:'chfl', index:'chfl', label:'出货分类', width:120, editable : true, edittype:"select", formatter:"select",
								editoptions:{value:"${fns:loadDictEditOption('CHFL_BS')}", dataInit:initSelect2}},
						 {name : 'spmc',index : 'spmc',label : '商品名称',width : 160},
						 {name : 'ddyqjhrq',index : 'ddyqjhrq',editable:false,label : '订单要求交货日期',width : 80,
						 ...
						 {name:'ckxhqk', index:'ckxhqk', label:'出口享惠情况', width:100, editable : true, edittype:"select", formatter:"select",
							editoptions:{value:"${fns:loadDictEditOption('CKXHQK')}", dataInit:initSelect2}},
	...
	function showDdxx(ddid){
		var url = "" ;
    	//备损
		url = "<c:url value='/order/orderSpo/viewPageByDh'/>" + "?ddid=" + ddid;
		if(url.length >  0){
			 window.open(url); 
		} 
	}
	...
	function initPiGrid(){
		$(grid_selector_pi).bindTable({
			...
			colModel : [ 
			             ...
			             {name : 'ddid',index : 'ddid',label : '订单号', editable : false, width : 100},
			             ...
			             {name : 'pilink',index : 'pilink',label : 'PI号',editable : false,width : 100,formatter:pixx},
	...
	function showPixx(piid){
		var url = "" ;
    	//备损
		url = "<c:url value='/pi/piSPO/viewPageByDh'/>" + "?piid=" + piid;
		if(url.length >  0){
			 window.open(url); 
		} 
	}
	...
	function initOtherGrid(){
		$(grid_selector_other).bindTable({
			...
			colModel : [ 
				...
				<shiro:hasPermission name="pso:spoPso:price">{name : 'je',index : 'je',label : '金额',width : 180,align:'right', formatter:'number',formatoptions:{thousandsSeparator: ',', decimalPlaces: 4}},</shiro:hasPermission>
	...
	function bindYzhdh(){
        $("#form-submit [name=ckyzhdh]").click(function(){
        	$.tableDialogPage({
    			...
    			url:"<c:url value='/pso/spoPso/search'/>" +"?yzhlx=2&zt=5" 
    		},{
    			callback:function(rows){
    				if(rows){
    					initValue(rows.id);
	...
	function bindZtyzh(){
		$("#form-submit [name=ztyzhdh]").parent().click(function(){
			$.tableDialogPage({
    			...
    			//选取主体预走货不做类型和 状态的限制
    			//url:"<c:url value='/pso/spoPso/search'/>" +"?yzhlx=1&zt=5"
	...
	function initFormDataByid(id){
		if(id != "null"){
			...
			$("#form-submit [name=yzhlx]").val("2");//预走货类型:大货预走货
	...
	function save(){
		...
		$.each(item, function(i, order){
			...
			if(je < piJe){
				swal("","订单："+order.ddid+"的金额不能小于PI明细金额！","warning");
				result = false;
				return false;
			}
		});
		...
		$("#save").bindSweetAlert({
			...
			callback:function(){
				$.bindAjax({
					url:"<c:url value='/pso/spoPso/edit'/>",
					data:param,
					action:"save",
				},function(response){
	...
	function submit(){
		...
		$("#submit").bindSweetAlert({
			...
			callback:function(){
				...
				$.bindAjax({
					url:"<c:url value='/pso/spoPso/submit'/>",
					data:param,
					action:"save",
				},function(response){
