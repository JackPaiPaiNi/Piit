	...
	<div class="page-content">
		<div class="row">
			<div class="col-xs-12">
					...
					<table class="table table-bordered" style="width: 1800px;">
						...
						<c:forEach items="${PsoVo.psoItemList}" var="item">
							<tr>
								...
								<td>${item.pbh}</td>
								<shiro:hasPermission name="pso:samplePso:price">
								<td><fmt:formatNumber value="${item.pdj}" pattern="#,##0.00"/></td>
								</shiro:hasPermission> --%>
								<td>${item.gtin}</td>
								<td>${item.cas}</td>
								<td>${item.dsjlxmc}</td>
				...
				<div class="row" style="margin: 0px 8px;">
					<table cellspacing="0" cellpadding="0">
						<tr>
							...
							<td style="width: 25%"><b>目的港:</b>${PsoVo.zzg}</td>
						</tr>
						<tr>
				...
				<%-- <h5 class="header blue" style="margin-top: 4px;">展会样机&认证样机</h5>
				<div class="row" style="margin: 0px 8px;">
					<table class="table table-bordered">
						<tr>
							<th>商品名称</th>
						...
						<c:forEach items="${PsoVo.psoExhibitionList}" var="item">
							<tr>
								<td>${item.spmc}</td>
								<td>${item.yjly}</td>
								<td>${item.jixing}</td>
...
<script type="text/javascript">
	$(function($) {
		...
		$('#btn-back').click(function() {
			//location.href = "<c:url value='/pso/samplePso'/>";
			window.history.back(-1);
		});
		//打印
		$("#btn-print").click(function(){
			window.open("<c:url value='/pso/samplePso/printPage'/>?id=" + $("input[name='id']").val());
		})
	});
	//打开订单查看界面
