				...
				<form id="form-submit">
					...
					<div class="row" style="margin: 0px 8px;">
						<div class="col-xs-6 col-sm-3">
								<div class="input-group input-group-sm">
									<label class="input-group-addon">发货公司&nbsp;&nbsp;</label>
						...
						<div class="col-xs-6 col-sm-3">
							<div class="input-group input-group-sm">
								...
								<select role="select" name="yzhlx"  size="1" class="form-control skyselect parent-node">
									${fns:loadDictOption('YZHLX_DYH')}
								</select>
							</div>
						</div>
					...
					<div class="row" style="margin: 0px 8px;">
						<div class="col-xs-6 col-sm-3">
							<div class="input-group input-group-sm">
								<label class="input-group-addon">是否白电&nbsp;&nbsp;</label>
								<div class="form-control">
									<label class="radio-inline">
										<input type="radio" class="skyradio skydisabled" disabled="disabled" name="sfBd" value="0" checked="checked">否
									</label>
									<label class="radio-inline">
										<input type="radio" class="skyradio skydisabled" disabled="disabled" name="sfBd" value="1">是
									</label>
								</div>
							</div>
						</div>
					...
					<div class="tab-content">
					    ...
					    <div class="space-4"></div>
					    <c:import url="/pub/showLog/diversityPso" charEncoding="UTF-8" />
					</div>
				</form>
				<!-- PAGE CONTENT ENDS -->
			</div>
...
<script type="text/javascript">
	...
	var isEdit = false;
	<shiro:hasPermission name="pso:diversityPso:edit">isEdit = true;</shiro:hasPermission>
	/******************************页面控件加载开始***************************************************/
	$(function($) {
		if(taskId != "null"){
			$('.approveSelect').show();
	...
	function initItmeGrid(){
		$(grid_selector_item).bindTable({
			...
			rowNum:50,
			recordtext: "",
			colModel : [ {name : 'id',label : 'ID',hidden : true,width : 60,editable : false,frozen:true} ,
			             ...
			             {name : 'cplxmc',index : 'cplxmc',label : '产品类型',width : 80,editable : false,frozen:true},
			             {name : 'cplx',index : 'cplx',hidden:true},
			             {name : 'jixing',index : 'jixing',editable:false,label : '物料编号',width : 80,frozen:true},
			             {name : 'model',index : 'model',label : '我司型号',width : 80},
			             {name : 'khxh',index : 'khxh',label : '客户型号',width : 80,editable : false},
			             {name : 'khxhms',index : 'khxhms',label : '产品描述',width : 80},
			             ...
			             {name : 'pp',index : 'pp',label : '品牌',width : 80,editoptions:{dataInit: initPP}},
				         ...
				         {name : 'mfjsl',index : 'mfjsl',label : '免费机数量', align:'right', width : 80, 
			            		 formatter:'integer',formatoptions:{thousandsSeparator: ','},editrules:{integer:true}},
				         {name : 'bz',index : 'bz',label : '币种',editable:false,width : 120}, 
				         <shiro:hasPermission name="pso:diversityPso:price">
						 ...
						 {name : 'fktj',index : 'fktj',hidden : true,editable:false,width : 80},
						 {name : 'fktjmc',index : 'fktjmc',hidden : true,label : '付款条件',editable:false,width : 80},
			             ...
			             {name : 'cplx',index : 'cplx',hidden:true,width : 80}, 
			             {name : 'zhfs',index : 'zhfs',hidden : true},
	...
	function showDdxx(ddid){
		var url = "" ;
    	//大货
		url = "<c:url value='/order/orderDiversity/viewPageByDh'/>" + "?ddid=" + ddid;
		if(url.length >  0){
			 window.open(url); 
		} 
	}
	...
	function showPixx(piid){
		var url = "" ;
    	//大货
		url = "<c:url value='/pi/piDiversity/viewPageByDh'/>" + "?piid=" + piid;
		if(url.length >  0){
			 window.open(url); 
		} 
	}
	...
	function bindYzhdh(){
        $("#form-submit [name=ckyzhdh]").click(function(){
        	$.tableDialogPage({
    			...
    			url:"<c:url value='/pso/diversityPso/search'/>?yzhlx="+$('#form-submit [name=yzhlx]').val()+"&zt=5"
    		},{
    			callback:function(rows){
    				if(rows){
    					initValue(rows.id);
	...
	function initValue(id){
		$.bindAjax({
			...
			action : "search"
			 ...
			 $("#form-submit [name=sfBd][value="+response.sfBd+"]").prop("checked","checked");
			 // 收货人信息
	...
	function bindZtyzh(){
		$("#form-submit [name=ztyzhdh]").parent().click(function(){
			$.tableDialogPage({
    			...
    			//选取主体预走货不做类型和 状态的限制
   				//url:"<c:url value='/pso/diversityPso/search'/>?yzhlx="+$('#form-submit [name=yzhlx]').val()+"&zt=5"
    		    url:"<c:url value='/pso/diversityPso/search'/>"
    		},{
    			callback:function(rows){
    				if(rows){
	...
	function initFormDataByid(id){
		if(id != "null"){
			...
			if(sfDyhp == "1"){
				$("#form-submit [name=yzhlx]").select2("val", "5");//预走货类型:多元化屏
			}else {
				$("#form-submit [name=yzhlx]").select2("val", "4");//预走货类型:多元化大货
			}
	...
	function save(){
		...
		$.each( item, function(i,order){  
            ...
            if(order.model == "" || order.model == null){
            	message += "第"+(i+1)+"行，我司型号不能为空.";
    			result = false;
            }
            if(order.chfl == "" || order.chfl == null){
            	message += "第"+(i+1)+"行，出货分类不能为空.";
    			result = false;
            }
		...
		$("#save").bindSweetAlert({
			...
			callback:function(){
				$.bindAjax({
					url:"<c:url value='/pso/diversityPso/edit'/>",
					data:param,
					action:"save",
				},function(response){
	...
	function submit(){
		...
		$("#submit").bindSweetAlert({
			...
			callback:function(){
				...
				$.bindAjax({
					url:"<c:url value='/pso/diversityPso/submit'/>",
					data:param,
					action:"save",
				},function(response){
	...
	function initDdid(){
      ...
      function initData(data){
    	   ...
    	   // 拆分客户型号描述
    	   /* $.each(data,function(index,tmpData){
    		   var words = tmpData.khxhms.split(";");
    		   tmpData.khxhms = words[0];
    		   tmpData.cpms = words[1];
    	   }) */
	       // 添加数据
	       if(localData.length > 0){
			   ...
			   $("#form-submit [name=mytk]").select2("val",data[0].gjmytk);
	     	   ...
	     	   $("#form-submit [name=sfBd][value="+data[0].sfBd+"]").prop("checked","checked");
	    	   $.bindAjax({
