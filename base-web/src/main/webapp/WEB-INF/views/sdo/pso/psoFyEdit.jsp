			...
			<div class="col-xs-12">
				...
				<form id="form-submit">
					...
					<div class="row" style="margin: 0px 8px;">
						...
						<div class="col-xs-6 col-sm-3">
							<div class="input-group input-group-sm">
								...
								<select role="select" name="yzhlx"  size="2" class="form-control skyselect parent-node">
									${fns:loadDictOption('YZHLX_FY')}
								</select>
							</div>
						</div>
					...
					<div class="tab-content">
					    ...
					    <div class="space-4"></div>
					    <c:import url="/pub/showLog/fyPso" charEncoding="UTF-8" />
					</div>
				</form>
				<!-- PAGE CONTENT ENDS -->
			</div>
...
<script type="text/javascript">
	...
	var isEdit = false;
	<shiro:hasPermission name="pso:fyPso:edit">isEdit = true;</shiro:hasPermission>
	/******************************页面控件加载开始***************************************************/
	$(function($) {
		if(taskId != "null"){
			$('.approveSelect').show();
	...
	function initItmeGrid(){
		$(grid_selector_item).bindTable({
			...
			colModel : [ {name : 'id',label : 'ID',hidden : true,width : 60,editable : false,frozen:true} ,
			             ...
			             {name : 'jixing',index : 'jixing',editable:false,label : '物料编号',width : 100,frozen:true},
				         ...
				         <shiro:hasPermission name="pso:fyPso:price">
	...
	function showDdxx(ddid){
		var url = "" ;
    	//大货
		url = "<c:url value='/order/orderFy/viewPageByDh'/>" + "?ddid=" + ddid;
		if(url.length >  0){
			 window.open(url); 
		} 
	}
	...
	function showPixx(piid){
		var url = "" ;
    	//大货
		url = "<c:url value='/pi/piFy/viewPageByDh'/>" + "?piid=" + piid;
		if(url.length >  0){
			 window.open(url); 
		} 
	}
	...
	function bindYzhdh(){
        $("#form-submit [name=ckyzhdh]").click(function(){
        	$.tableDialogPage({
    			...
    			url:"<c:url value='/pso/fyPso/search'/>?yzhlx="+$('#form-submit [name=yzhlx]').val()+"&zt=5"
    		},{
    			callback:function(rows){
    				if(rows){
    					initValue(rows.id);
	...
	function bindZtyzh(){
		$("#form-submit [name=ztyzhdh]").parent().click(function(){
			$.tableDialogPage({
    			...
    			//选取主体预走货不做类型和 状态的限制
   				//url:"<c:url value='/pso/fyPso/search'/>?yzhlx="+$('#form-submit [name=yzhlx]').val()+"&zt=5"
    		    url:"<c:url value='/pso/fyPso/search'/>"
    		},{
    			callback:function(rows){
    				if(rows){
	...
	function initFormDataByid(id){
		if(id != "null"){
			...
			$("#form-submit [name=yzhlx]").select2("val", "7");//预走货类型:多元化大货
	...
	function save(){
		...
		$("#save").bindSweetAlert({
			...
			callback:function(){
				$.bindAjax({
					url:"<c:url value='/pso/fyPso/edit'/>",
					data:param,
					action:"save",
				},function(response){
	...
	function submit(){
		...
		$("#submit").bindSweetAlert({
			...
			callback:function(){
				...
				$.bindAjax({
					url:"<c:url value='/pso/fyPso/submit'/>",
					data:param,
					action:"save",
				},function(response){
	...
	function initDdid(){
			...
			var pageUrl = "<c:url value='/pso/tvPso/ddcxPage'/>?&yzhlx="+$("#form-submit [name=yzhlx]").val();
