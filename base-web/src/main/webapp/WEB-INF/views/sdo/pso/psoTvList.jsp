...
<script type="text/javascript">
	...
	var isEdit = false;
	var editurl = "<c:url value='/pso/tvPso/editPage'/>";
	var viewurl = "<c:url value='/pso/tvPso/viewPage'/>";
	var delurl = "<c:url value='/pso/tvPso/delete'/>";
	var changeurl = "<c:url value='/pso/tvPso/change'/>";
	var cancelurl = "<c:url value='/pso/tvPso/cancel'/>";
	var getbackurl = "<c:url value='/pso/tvPso/getback'/>";
	...
	$(function($) {
		...
		$(grid_selector).bindTable({
			...
			colModel : [ 
	             ...
	             { name : 'bbh', label : 'bbh', hidden : true, width : 60}, 
	             { name : 'sjc', label : 'sjc', hidden : true, width : 60}, 
	             { name : 'cysfbl', label : 'cysfbl', hidden : true, width : 60}, 
	             { name : 'processId', label : 'processId', hidden : true, width : 60, editable : false }, 
	             { name : 'scjdmc', index : 'scjdmc', label : '生产基地', width : 80 }, 
	             { name : 'yzhdh', index : 'yzhdh', label : '预走货单号', width : 120 }, 
	             { name : 'zt', index : 'zt', label : '状态', width : 60, hidden : true}, 
		...
		<shiro:hasPermission name="pso:tvPso:edit">
			...
			$(grid_selector).navButtonAdd(pager_selector,{
				...
				onClickButton: function(){
					...
					if(id==null || id==""){
						...
						var rowData = jQuery(grid_selector).jqGrid('getRowData',id);
						if(rowData.zt == 2 || rowData.zt == 4 || rowData.zt == 3 || (rowData.zt == 1 && rowData.cysfbl == 0)){
							$("body").bindSweetAlert({
								title:"确定要取回吗?"
							},{
								callback:function(){
		...
		<shiro:hasPermission name="pso:tvPso:smoApprove">
		$(grid_selector).navButtonAdd(pager_selector,{
			...
			onClickButton: function(){
				...
				if(id==null || id==""){
					...
					$.bindAjax({
						url:"<c:url value='/pso/tvPso/smoApprove'/>",
						data:{id:id, sjc:rowData.sjc},
						action:"save"
					},function(response){
						$(grid_selector).trigger("reloadGrid");
	...
	// 新增
	function addfunc() {
		location.href = "<c:url value='/pso/tvPso/editPage'/>";
	}
	...
	function editfunc() {
		...
		if(selr==null||selr==""){
			...
			if(zt == 1 && (rowData.cysfbl == "" || rowData.cysfbl == 1)){
				var id = jQuery(grid_selector).jqGrid('getGridParam', 'selrow');
			...
			if (rowData.zt != 1 && rowData.zt != 3) {
				...
				$("body").bindSweetAlert({
	    			...
	    			callback:function(){
	    				$.bindAjax({
	    					url:delurl,
	    					data:{id : rowData.id, sjc : rowData.sjc, zt : rowData.zt, processId : processId},
	    					action:"edit"
	    				},function(response){
	    					$(grid_selector).trigger("reloadGrid");
	    				}); 
