			...
			<div id="search-box" class="widget-box">
				...
				<div class="widget-body">
					<form id="form-search" name="form-search" class="form-search">
                         <div class="row">
							<input type ="hidden" value ="14" name ="lx">
						</div>	
					</form>
				</div>
			</div>
...
$(function($) {
	...
	var gridTable = $(grid_selector).bindTable({
		...
		colModel: [
			...
			{name:'fph', index:'fph', label:'发票'},
			{name:'ddid', index:'ddid', label:'订单号'},
			{name:'fprq', index:'fprq', label:'发票日'},
			{name:'mytkmc', index:'mytkmc', label:'贸易条款'},
			{name:'fpdqr', index:'fpdqr', label:'发票到期日'},
			{name:'sjhkrq', index:'sjhkrq', label:'实际回款日期'},
