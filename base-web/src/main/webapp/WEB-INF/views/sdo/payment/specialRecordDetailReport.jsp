			...
			<div id="search-box" class="widget-box">
				...
				<div class="widget-body">
					<form id="form-search" name="form-search" class="form-search">
                         <div class="row">
							<input type ="hidden" value ="13" name ="lx">
						</div>	
					</form>
				</div>
			</div>
...
$(function($) {
	...
	var gridTable = $(grid_selector).bindTable({
		...
		rowNum : 20,
		colModel: [
			{name:'tpsj', index:'tpsj', label:'特批时间'},
			{name:'khmc', index:'khmc', label:'客户'},
			{name:'ddid', index:'ddid', label:'订单号'},
			{name:'tpjdmc', index:'tpjdmc', label:'特批节点'},
			{name:'tplxmc', index:'tplxmc', label:'特批类型'},
			{name:'bz', index:'bz', label:'币种'},
			{name:'tpje', index:'tpje', label:'特批金额'},
			{name:'sf_ysc', index:'sf_ysc', label:'特批状态'},
			{name:'yxcje', index:'yxcje', label:'已消除金额'},
			{name:'wxcje', index:'wxcje', label:'未消除金额'},
			{name:'yzhdh', index:'yzhdh', label:'预走货号'},
			{name:'fph', index:'fph', label:'发票号'},
			{name:'fpje', index:'fpje', label:'发票金额'},
			{name:'zgsj', index:'zgsj', label:'装柜日期'},
			{name:'pfbl', index:'pfbl', label:'赔付比例'}
		]
	},{
		add:false,
		edit:false,
