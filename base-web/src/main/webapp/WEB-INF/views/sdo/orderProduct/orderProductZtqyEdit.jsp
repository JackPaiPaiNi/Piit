				...
				<div class="row" style="margin: 0px 8px;">
					...
					<table cellspacing="0" cellpadding="0">
						<tr>
							...
							<td style="width: 25%">
								<c:if test="${sfZt == 1}">
									<button id="btn-pause" type="button" class="btn btn-info btn-sm" onclick="javascript:pause(1);">
										<i class="icon-ok icon-on-right bigger-110"></i>
										暂停
									</button>
								</c:if>
								<c:if test="${sfZt == 2}">
									<button id="btn-start" type="button" class="btn btn-info btn-sm" onclick="javascript:pause(0);">
										<i class="icon-ok icon-on-right bigger-110"></i>
										启用
									</button>
								</c:if>
								<button id="btn-submit" type="button" class="btn btn-info btn-sm submitSelect" style="display:none">
									<i class="icon-ok icon-on-right bigger-110"></i>
									提交
								</button>
...
<%--								<tr>
				...
				<div class="row" style="margin: 0px 8px;">
					<table cellspacing="0" cellpadding="0">
						<tr>
							<td colspan="2" style="width: 50%;"><b>美工任务单号：</b>${orderProduct.mgrwdh}</td>
...
<script type="text/javascript">
	...
	function pause(type){
		...
		$("body").bindSweetAlert({
			...
			callback:function(){
				$.bindAjax({
					url:"<c:url value='/order/orderProductNew/pauseSubmit'/>",
					data:param,
					action:"save"
				},function(response){
					window.history.back(-1);
