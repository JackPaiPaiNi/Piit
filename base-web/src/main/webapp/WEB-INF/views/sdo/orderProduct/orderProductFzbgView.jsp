		...
		<div class="col-xs-12">
			...
			<form id="form-submit">
				<div class="row" style="margin: 0px 8px;">
					...
					<input type="hidden" name="bbh" value="${orderProduct.bbh}"/>
							...
							<td style="width: 40%">
								...
								<button id="btn-print" type="button" class="btn btn-light btn-sm">
								 ...
								 <c:if test="${sfApprove != 1}">
									<%-- <c:if test="${orderProduct.zt == '4' || orderProduct.zt == '5'}">
										<button id="btn-print" type="button" class="btn btn-light btn-sm">
											<i class="icon-print icon-on-right bigger-110"></i>
											打印
										</button>
				...
				<h5 class="header blue" style="margin: 0; padding: 0;">基本信息</h5>
				<div class="row" style="margin: 0px 8px;">
					<table cellspacing="0" cellpadding="0">
						<tr>
						...
						<shiro:hasPermission name="order:orderProduct:price">
						<tr>
							<td><b>单价：</b><fmt:formatNumber value="${orderProduct.dj}" pattern="#,##0.00"/></td>
							<td><b>金额：</b><fmt:formatNumber value="${orderProduct.je}" pattern="#,##0.00"/></td>								
							<td><b>付费备损金额：</b><fmt:formatNumber value="${orderProduct.ffbsje}" pattern="#,##0.00"/></td>
							<td><b>总金额（含备损）：</b><fmt:formatNumber value="${orderProduct.zje}" pattern="#,##0.00"/></td>
						</tr>
						</shiro:hasPermission>
						<tr>
						...
						<tr>
							<td><b>走货码：</b>${orderProduct.zhmmc}</td>
							<td><b>版本号：</b>${orderProduct.versionmc}</td>
							<td><b>标识号：</b>${orderProduct.bsmc}</td>
							<td></td>
						</tr>
							<tr>
							<td><b>走货码：</b>${orderProduct.zhmmct}</td>
							<td><b>版本号：</b>${orderProduct.versionmct}</td>
							<td><b>标识号：</b>${orderProduct.bsmct}</td>
							<td></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td><b>电视机类型：</b>${orderProduct.dsjlxmc}</td>
						</tr>
						<tr>
							<td colspan="4"><b>基本信息备注：</b><PRE class="pre">${orderProduct.jbxxbz}</PRE></td>
						</tr>
						<%-- <tr>
							<td colspan="4"><b>辅助变更备注：</b><PRE class="pre">${orderProduct.fzbgbz}</PRE></td>
						</tr> --%>
					</table>
				</div>
				<!-- 客户信息TAB -->
				...
				<h5 class="header blue" style="margin: 0; padding: 0;">产品信息</h5>
					...
					<div class="row" style="margin: 0px 8px;">
						<table cellspacing="0" cellpadding="0">
							...
							<tr>
								<td style="width: 25%;"><b>屏是否需要保护膜：</b>
									<input type="radio" class="skyradio" <c:if test="${orderProduct.sfBhm == 1}">checked="checked"</c:if>>需要
									<input type="radio" class="skyradio" <c:if test="${orderProduct.sfBhm == 0}">checked="checked"</c:if>>不需要
								</td>
								<td>&nbsp;</td>
								<td>&nbsp;</td>
								<td>&nbsp;</td>
					...
					<div class="row" style="margin: 0px 8px;">
						<table cellspacing="0" cellpadding="0">
							...
							<tr>
								<td colspan="4"><b>其他附件备注：</b>${orderProduct.qtfjbz}</td>
							</tr>
						</table>
					</div>
					...
					<div class="row" style="margin: 0px 8px;">
						<table cellspacing="0" cellpadding="0">
							...
							<tr>
								<td colspan="4"><b>SKD前壳需加工配件：</b><PRE class="pre">${orderProduct.skdqkxjgpj}</PRE></td>
							</tr>
							<tr>
								<td colspan="2"><b>S/H/CKD加工要求：</b><PRE class="pre">${orderProduct.shckdjgyq}</PRE></td>
					...
					<div class="row" style="margin: 0px 8px;">
						<table cellspacing="0" cellpadding="0">
							<tr>
								<td><b>产品其他要求备注：</b><PRE class="pre">${orderProduct.cpqtyqbz}</PRE></td>
							</tr>
							<tr>
				...
				<!-- </div> -->
				<!-- 美工信息TAB -->
				<h5 class="header blue" style="margin: 0; padding: 0;">美工信息</h5>
				<div class="row" style="margin: 0px 8px;">
					<table cellspacing="0" cellpadding="0">
						<tr>
							<td colspan="2" style="width: 50%;"><b>美工任务单号：</b>
								<a style="cursor: pointer;" onclick="javascript:showRWD('${orderProduct.mgrwdh}');">${orderProduct.mgrwdh}</a>
							</td>
							<td style="width: 25%;"><b>序列号类型：</b>${orderProduct.xlhmc}</td>
							<td style="width: 25%;"><b>序列号码：</b>${orderProduct.xlhbz}</td>
						</tr>
						<tr>
							<td colspan="4"><b>序列号其他：</b>${orderProduct.xlhqt}</td>
						</tr>
						<tr>
				...
				<h5 class="header blue" style="margin: 0; padding: 0;">备损物料信息</h5>
					...
					<div class="row" style="margin: 0px 8px;">
						<table cellspacing="0" cellpadding="0">
							...
							<tr>
								<td colspan="4"><b>备损备注：</b><PRE class="pre">${orderProduct.bsxxbz}</PRE></td>
							</tr>
						</table>
					</div>
...
<script type="text/javascript">
	$(function($){
		...
		$("#btn-print").click(function(){
			window.open("<c:url value='/order/orderProductNew/printPage'/>?id=" + $("input[name='id']").val());
		})
		// 查看流程
	...
	//打开任务单查看页面
	function showRWD(rwdh){
		var url = "" ;
		url = "<c:url value='/order/orderProductNew/rwdviewPage'/>" + "?rwdh=" + rwdh + "&sfView=0";
		if(url.length >  0){
			 window.open(url); 
		} 
	}
