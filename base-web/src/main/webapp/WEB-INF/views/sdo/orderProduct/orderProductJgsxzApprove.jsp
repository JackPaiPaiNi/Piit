...
$(function($) {
	var sfWxdd = $("#form-submit [name=sfWxdd]").val();
	var dzcode = "";
	var dycode = "";
	var jgcode = "";
	if(sfWxdd == 1){//外协
		...
		dycode = "1-yx-wxdysjs";
		jgcode = "1-yx-wxjgsjs";
	}else if(sfWxdd == 0){//非外协
		dzcode = "3-yf-dzsjs";
		dycode = "3-yf-dysjs";
		jgcode = "3-yf-jgsjs";
	}
	//加载电子工程师
	$.post("<c:url value='/pub/select2/selectUserByRole'/>",{roleCode:dzcode},
			function(result){
	...
	//加载电源工程师
	$.post("<c:url value='/pub/select2/selectUserByRole'/>",{roleCode:dycode},
			function(result){
				var data = $.map(result, function (obj) {
			       obj.id = obj.empCode;
			       obj.text = obj.text || obj.userName;	
	...
	//加载结构工程师
	$.post("<c:url value='/pub/select2/selectUserByRole'/>",{roleCode:jgcode},
			function(result){
				var data = $.map(result, function (obj) {
			       obj.id = obj.empCode;
			       obj.text = obj.text || obj.userName;	
...
<%--								<tr>
		...
		<div class="space-4"></div>
			...
			<div class="row" id="jpcl" style="margin: 0px 8px;">
				<div class="col-xs-6 col-sm-3">
					<div class="input-group input-group-sm">
						<label class="input-group-addon">是否加批&nbsp;&nbsp;</label>
						<div class="form-control">
							<label class="radio-inline">
								<input type="radio" class="skyradio kczRadio" name="sfjp" value="1">是
							</label>
							<label class="radio-inline">
								<input type="radio" class="skyradio kczRadio" name="sfjp" value="0" checked="checked">否
							</label>
						</div>
					</div>
				</div>
...
<script type="text/javascript">
	...
	function approve(type){
		...
		var dzgcs = $("#form-submit [name=dzgcs]").val();
		var dygcs = $("#form-submit [name=dygcs]").val();
		var jggcs = $("#form-submit [name=jggcs]").val();
		if(type == 1 && (dzgcs.length == 0 || dygcs.length == 0 || jggcs.length == 0)){
			swal("","请选择电子设计师、电源设计师、结构设计师！","warning");
			return;
		}
		$("body").bindSweetAlert({
			title:"确定要提交吗?"
			...
			callback:function(){
				$.bindAjax({
					url:"<c:url value='/order/orderProductNew/approve'/>",
					data:param,
					action:"save"
				},function(response){
