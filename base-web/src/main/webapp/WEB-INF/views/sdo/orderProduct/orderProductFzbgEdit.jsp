			...
			<form id="form-submit">
				...
				<div class="tab-content">
					...
					<div class="tab-pane fade active in" id="jbxx-tab">
						...
						<div class="col-xs-6 col-sm-3">
								<div class="input-group input-group-sm">
									...
									<select id = "zhm" role="select" name="zhm" size="1" class="form-control skydisabled" readonly>
					...
					<div class="tab-pane fade" id="mgxx-tab">
						<div class="row" style="margin: 0px;">
							<p style="margin-left: 10px; color: red;">修改美工任务单时若影响PID变化，请先取消此大货订单，再重新下新的大货订单！</p>
						    <div class="col-xs-6 col-sm-6">
								<div class="input-group input-group-sm">
									<label class="input-group-addon">美工任务单号&nbsp;&nbsp;</label>
									<input type="text" name="mgrwdh" onfocus="this.blur()" class="form-control parent-node"/>
									<span class="input-group-addon">
										<i class="icon-search bigger-110"></i>
									</span>						
							    </div>
...
<script type="text/javascript">
	...
	function initPageData(id){
		if(id == "null"){
			...
			$.loading();
			$.bindAjax({
				url : "<c:url value='/order/orderProductNewFzbg/findById'/>",
				data : {id : id},
				action : "search"
			},function(response){
				//加载加工方式
	...
	function save(){
		...
		$("body").bindSweetAlert({
			...
			callback:function(){
				$.bindAjax({
					url:"<c:url value='/order/orderProductNewFzbg/edit'/>",
					data:param,
					action:"save"
				},function(response){
	...
	function submit(){
		...
		$("body").bindSweetAlert({
			...
			callback:function(){
				$.bindAjax({
					url:"<c:url value='/order/orderProductNewFzbg/submit'/>",
					data:param,
					action:"save"
				},function(response){
					window.history.back(-1);
