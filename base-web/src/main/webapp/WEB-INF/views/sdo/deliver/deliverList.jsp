...
<script type="text/javascript">
	...
	$(function($) {
		...
		$(grid_selector).bindTable({
			caption: "",
			url: "<c:url value='/deliver/deliver/search'/>",
			...
			colModel: [
				...
				{name:'zje', index:'zje', label:'总金额', width:80, align:'right', formatter:'integer', formatoptions:{thousandsSeparator:','}},
				...
				{name:'jfxtsj', index:'jfxtsj', label:'截放行条时间', width:90, formatter:'date', formatoptions:{newformat:'Y-m-d'}},
				{name:'bgfsmc', index:'bgfsmc', label:'报关方式', width:80},
				{name:'fph', index:'fph', label:'发票号', width:90},
				...
				{name:'tssapzt', index:'tssapzt', label:'推送SAP状态', width:100, formatter:tsSAPCellAddHref},
				{name:'sfYfyj', index:'sfYfyj', label:'是否已发邮件', width:100, edittype:"select", formatter: "select", editoptions:{value:"${fns:loadDictEditOption('SF')}"}},
		...
		/* $(grid_selector).navButtonAdd(pager_selector,{
			...
			onClickButton: function(){
				...
				if(id==null || id==""){
					...
					$("body").bindSweetAlert({
		    			title:"取消后将不能再使用此出货通知书单号，确定要取消出货通知书？",
		    			closeOnConfirm: true
		    		},{
		    			callback:function(){
		    				//加载层-风格2
		    				$.bindAjax({
								url:"<c:url value='/deliver/deliver/cancel'/>",
