...
<script type="text/javascript">
	...
	var editurl = "<c:url value='/order/orderDiversity/editPage'/>";
	var viewurl = "<c:url value='/order/orderDiversity/viewPage'/>";
	var changeurl = "<c:url value='/order/orderDiversity/change'/>";
	var cancelurl = "<c:url value='/order/orderDiversity/cancel'/>";
	var getbackurl = "<c:url value='/order/orderDiversity/getback'/>";
	var isEdit = false;
	var isChdelete = false;
	<shiro:hasPermission name="order:orderDiversity:chdelete">isChdelete=true;</shiro:hasPermission>
	<shiro:hasPermission name="order:orderDiversity:edit">isEdit=true;</shiro:hasPermission>
	...
	$(function($) {
		...
		$(grid_selector).bindTable({
			caption: "",
			url: "<c:url value='/order/orderDiversity/search'/>",
			...
			colModel: [
				...
				{name:'scjdmc', index:'scjdmc', label:'发货公司', width:60, frozen:true},
		...
		<shiro:hasPermission name="order:orderDiversity:edit">
			...
			$(grid_selector).navButtonAdd(pager_selector,{
				...
				onClickButton: function(){
					...
					if(id==null || id==""){
						...
						$.bindAjax({
							url:"<c:url value='/order/orderDiversity/pushSAP'/>",
		...
		$("#btn-export").click(function(){
		    ...
		    if(curNum==0){
	        	...
	        	$("#export").bindSweetAlert({
	    			...
	    			callback:function(){
	    				$(grid_selector).exportExcel({url: "<c:url value='/order/orderDiversity/export'/>"});
	    			}
	    		});
	        }
		});
		...
		if(selr == null || selr == ""){
			...
			if (zt != 1 && zt != 3) {
				...
				$("body").bindSweetAlert({
					...
					callback:function(){
						$.bindAjax({
							url:"<c:url value='/order/orderDiversity/delete'/>",
