...
<script type="text/javascript">
	...
	var editurl = "<c:url value='/order/orderProduct/editPage'/>";
	var viewurl = "<c:url value='/order/orderProduct/viewPage'/>";
	var changeurl = "<c:url value='/order/orderProduct/change'/>";
	var cancelurl = "<c:url value='/order/orderProduct/cancel'/>";
	var getbackurl = "<c:url value='/order/orderProduct/getback'/>";
	var changeEditurl = "<c:url value='/order/orderProduct/changeEditPage'/>";
	var pauseurl = "<c:url value='/order/orderProduct/pause'/>";
	var pauseEditurl = "<c:url value='/order/orderProduct/pauseEditPage'/>";
	var isEdit = false;
	var isChdelete = false;
	<shiro:hasPermission name="order:orderProduct:chdelete">isChdelete=true;</shiro:hasPermission>
	...
	$(function($) {
		...
		$(grid_selector).bindTable({
			caption: "",
			url: "<c:url value='/order/orderProduct/search'/>",
		...
		<shiro:hasPermission name="order:orderProduct:edit">
			...
			$(grid_selector).navButtonAdd(pager_selector,{
				...
				onClickButton: function(){
					...
					if(id==null || id==""){
						...
						$.bindAjax({
							url:"<c:url value='/order/orderProduct/pushSAP'/>",
			...
			/* $(grid_selector).navButtonAdd(pager_selector,{
				...
				onClickButton: function(){
					...
					if(id==null || id==""){
						...
						var rowData = jQuery(grid_selector).jqGrid('getRowData',id);
						$(grid_selector_ckd).exportExcelNoData({url: "<c:url value='/order/orderProduct/ckdExport'/>"+"?id="+rowData.id});
					}
				},
				position:"last"
			}); */
		...
		if(selr == null || selr == ""){
			...
			if (zt != 1 && zt != 3 ) {
				...
				$("body").bindSweetAlert({
					...
					callback:function(){
						$.bindAjax({
							url:"<c:url value='/order/orderProduct/delete'/>",
