							...
							<div class="row">
								...
								<div class="col-xs-6 col-sm-3">
									<div class="input-group input-group-sm">
										...
										<select role="select" name="sfWx" size="1" class="form-control skyselect">
											${fns:loadDictOption('SFWX')}
										</select>
									</div>
								</div>
							</div>
...
<script type="text/javascript">
	...
	var editurl = "<c:url value='/order/orderSample/editPage'/>";
	var viewurl = "<c:url value='/order/orderSample/viewPage'/>";
	var changeurl = "<c:url value='/order/orderSample/change'/>";
	var cancelurl = "<c:url value='/order/orderSample/cancel'/>";
	var getbackurl = "<c:url value='/order/orderSample/getback'/>";
	var changeEditurl = "<c:url value='/order/orderSample/changeEditPage'/>";
	var isEdit = false;
	var isChdelete = false;
	<shiro:hasPermission name="order:orderSample:chdelete">isChdelete=true;</shiro:hasPermission>
	...
	$(function($) {
		...
		$(grid_selector).bindTable({
			caption: "",
			url: "<c:url value='/order/orderSample/search'/>",
			...
			colModel: [
				...
				{name:'ddid', index:'ddid', label:'订单号', width:100, editoptions:{readonly:"true"}, frozen:true},
				...
				{name:'sl', index:'sl', label:'订单数量', width:80, align:'right',formatter:'integer',formatoptions:{thousandsSeparator: ','}},
				...
				{name:'yddid', index:'yddid', label:'原订单号', width:80, editoptions:{readonly:"true"}},
		...
		<shiro:hasPermission name="order:orderSample:edit">
		...
		<shiro:hasPermission name="order:orderSample:tssap">
		$(grid_selector).navButtonAdd(pager_selector,{
			caption:"推送SAP", 
			buttonicon:"fa-pencil-square grey", 
			onClickButton: function(){
				...
				if(id==null || id==""){
					...
					$.bindAjax({
						url:"<c:url value='/order/orderSample/pushSAP'/>",
		...
		$("#btn-export").click(function(){
		    ...
		    if(curNum==0){
	        	...
	        	$("#export").bindSweetAlert({
	    			...
	    			callback:function(){
	    				$(grid_selector).exportExcel({url: "<c:url value='/order/orderSample/export'/>"});
	    			}
	    		});
	        }
		});
		...
		if(selr == null || selr == ""){
			...
			if (zt != 1 && zt != 3) {
				...
				$("body").bindSweetAlert({
					...
					callback:function(){
						$.bindAjax({
							url:"<c:url value='/order/orderSample/delete'/>",
