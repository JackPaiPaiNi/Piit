							...
							<div class="row">
								...
								<div class="col-xs-6 col-sm-3">
									<div class="input-group input-group-sm">
										...
										<select role="select" name="ddlb" size="2" class="form-control skyselect">
...
<script type="text/javascript">
	...
	var editurl = "<c:url value='/order/orderFy/editPage'/>";
	var viewurl = "<c:url value='/order/orderFy/viewPage'/>";
	var changeurl = "<c:url value='/order/orderFy/change'/>";
	var cancelurl = "<c:url value='/order/orderFy/cancel'/>";
	var getbackurl = "<c:url value='/order/orderFy/getback'/>";
	var isEdit = false;
	var isChdelete = false;
	<shiro:hasPermission name="order:orderFy:chdelete">isChdelete=true;</shiro:hasPermission>
	<shiro:hasPermission name="order:orderFy:edit">isEdit=true;</shiro:hasPermission>
	...
	$(function($) {
		...
		$(grid_selector).bindTable({
			caption: "",
			url: "<c:url value='/order/orderFy/search'/>",
			...
			colModel: [
				...
				{name:'scjdmc', index:'scjdmc', label:'工厂', width:60, frozen:true},
				...
				{name:'zt', index:'zt',label:'订单状态', hidden:true, width:80, frozen:true},
				{name:'ztmc', index:'ztmc',label:'订单状态', width:80, frozen:true},
				{name:'ddlb', index:'ddlb', label:'订单类别', hidden:true},
				...
				{name:'zsl', index:'zsl',align:'right', label:'订单数量', width:80,formatter:'integer',formatoptions:{thousandsSeparator: ','}},
				{name:'zmfsl', index:'zmfsl',align:'right', label:'订单免费数量', width:90,formatter:'integer',formatoptions:{thousandsSeparator: ','}},
				...
				{name:'zje', index:'zje', align:'right',label:'总金额', width:80,formatter:'number',formatoptions:{thousandsSeparator: ','}},
				...
				{name:'zhfsmc', index:'zhfs', label:'走货方式', width:80},
		...
		<shiro:hasPermission name="order:orderFy:edit">
						...
						if(rowData.zt != 5){
							...
							/* $("body").bindSweetAlert({
								...
								callback:function(){
									$.bindAjax({
										url:cancelurl,
										data:{id:id, sjc:rowData.sjc, ddid:rowData.ddid, ddlb:rowData.ddlb, type:1},
										action:"edit"
									},function(response){
										$(grid_selector).trigger("reloadGrid");
									});
			...
			$(grid_selector).navButtonAdd(pager_selector,{
				...
				onClickButton: function(){
					...
					if(id==null || id==""){
						...
						$.bindAjax({
							url:"<c:url value='/order/orderFy/pushSAP'/>",
		...
		$("#btn-export").click(function(){
		    ...
		    if(curNum==0){
	        	...
	        	$("#export").bindSweetAlert({
	    			...
	    			callback:function(){
	    				$(grid_selector).exportExcel({url: "<c:url value='/order/orderFy/export'/>"});
	    			}
	    		});
	        }
		});
		...
		if(selr == null || selr == ""){
			...
			if (zt != 1 && zt != 3) {
				...
				$("body").bindSweetAlert({
					...
					callback:function(){
						$.bindAjax({
							url:"<c:url value='/order/orderFy/delete'/>",
	...
	function initDlg(rowData,type){
		...
		$('#consoleDlg [name=ddid]').val(rowData.ddid);
