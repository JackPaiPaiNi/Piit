	...
	<div class="row">
		<div class="col-xs-12">
			...
			<form id="form-submit">
				...
				<div class="row" style="margin: 0px 8px;">
					...
					<div class="col-xs-6 col-sm-3">
						<div class="input-group input-group-sm">
							...
							<select role="select" name="ddlx" size="1" class="form-control skydisabled" disabled="disabled">
				...
				<div class="row" style="margin: 0px 8px;">
					...
					<div class="col-xs-6 col-sm-3" >
						<div class="input-group input-group-sm">
							...
							<select role="select" name="sfYh" size="1" class="form-control skyselect">
					...
					<div class="col-xs-6 col-sm-3">
						<div class="input-group-sm">
							<input type="text" id="sfyhbz" name="sfyhbz" placeholder="是否验货备注" class="form-control"/>
						</div>
					</div>
				</div>
				<div class="space-4"></div>
			    ...
			    <div class="space-4"></div>
			    <c:import url="/pub/showLog/orderDiversity" charEncoding="UTF-8" />
			</form>
			<!-- PAGE CONTENT ENDS -->
		</div><!-- /.col -->
	</div><!-- /.row -->
...
<script type="text/javascript">
	...
	var isEdit = false;
	var hasDelete = false;
	<shiro:hasPermission name="order:orderDiversity:edit">isEdit=true;hasDelete=true;</shiro:hasPermission>
	...
	$(function($){
		...
		$("#btn-piSelect").click(function(){
			...
			var pageUrl = "<c:url value='/order/orderReferPi/diversity'/>"+"?ddid=" + ddid;
		...
		$(grid_cpxx_selector).bindTable({
			...
			colModel: [
				...
				{name:'cplx', index:'cplx', label:'产品类型', width:80, editable : true, edittype:"select", formatter:"select",
					editoptions:{value:"${fns:loadDictEditOption('CPLX')}", dataInit:initSelect2}},
	...
	function initFormData(id){
		if(id == "null"){
			...
			$("#form-submit [name=ddlx]").val("4");
			if(sfDyhp == "1"){
				$("#form-submit [name=ddlb]").select2("val","4");//订单类别:多元化屏
			}else {
				$("#form-submit [name=ddlb]").select2("val","1");//PI类型:多元化大货
			}
			...
			// edit
			$.bindAjax({
				url : "<c:url value='/order/orderDiversity/findById'/>",
				data : {id : id},
				action : "search"
			},function(response){
				if(response){
					...
					bindGridCpxx();
					// 加载产品信息明细
					if(response.orderDiversityItemList){
						$(grid_cpxx_selector).jqGrid("addRowData", null, response.orderDiversityItemList, "last");
					}
	...
	function save(){
		...
		$("body").bindSweetAlert({
			...
			callback:function(){
				$.bindAjax({
					url:"<c:url value='/order/orderDiversity/edit'/>",
					data:param,
					action:"save"
				},function(response){
					if(response){
	...
	function submit(){
		...
		$("body").bindSweetAlert({
			...
			callback:function(){
				$.bindAjax({
					url:"<c:url value='/order/orderDiversity/submit'/>",
					data:param,
					action:"save"
				},function(response){
					window.history.back(-1);
