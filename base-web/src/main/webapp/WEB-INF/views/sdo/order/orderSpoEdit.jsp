		...
		<div class="col-xs-12">
			...
			<form id="form-submit">
				<div class="row" style="margin: 0px 8px;">
				    ...
				    <input type="hidden" name="sfKszy" value="${sfKszy}"/>
					...
					<div class="col-xs-6 col-sm-3">
						<div class="input-group input-group-sm">
							...
							<select id="ddlb" role="select" name="ddlb" size="1" class="form-control skyselect">
						...
						<div class="input-group input-group-sm">
							...
							<div class="form-control">
								<label class="radio-inline">
									<input type="radio" class="skyradio" name="sfCzz" value="1" checked="checked">是
								</label>
								<label class="radio-inline">
									<input type="radio" class="skyradio"  name="sfCzz" value="0" >否
								</label>
							</div>
						</div>
					</div>
				...
				<div class="row" style="margin: 0px 8px;">
					...
					<div class="col-xs-6 col-sm-3">
						<div class="input-group input-group-sm">
							...
							<select role="select" name="zrhf" size="1" class="form-control skyselect skydisabled" disabled="disabled">
				...
				<div class="row" style="margin: 0px 8px;">
				   ...
				   <div class="col-xs-6 col-sm-3">
						<div class="input-group input-group-sm">
							...
							<input type="text" name="fktjmc" onfocus="this.blur()" class="form-control parent-node"/>
							<span class="input-group-addon">
								<i class="icon-search bigger-110"></i>
							</span>
						</div>
			    ...
			    <div class="space-4"></div>
			    <c:import url="/pub/showLog/orderSpo" charEncoding="UTF-8" />
			</form>
...
<script type="text/javascript">
	...
	$(function($){
		...
		$("#form-submit [name=khbm]").click(function(){
			...
			if(xsyid == null || xsyid == ""){
	    		...
	    		return ;
			}else if(ddlb == null || ddlb == ""){
				swal('','请先选择订单类别！','warning'); 
	    		return ;
			} else {
				bindKhbm();
			}
	    ...
	    $("#form-submit [name=ddlb]").on("change", function(e) {
	    	...
	    	var  sfKszy = $('#form-submit [name=sfKszy]').val() ;
	    	if(ddlb==3 && sfKszy==0 ){//(ddlb == 11 ||  ddlb==3) && sfKszy==0
				swal("", "只有客诉专员才能下 技术服务部客诉备损，请重新选择订单类别!", "warning");//供应商免费提供备损  剔除
				 $("#form-submit [name=ddlb]").select2('val','');
				return;
			}
	    	//供应商免费提供备损不穿制造
			if(ddlb == 11){
				$("#form-submit [name='sfCzz'][value=1]").removeProp("checked").removeAttr("checked"); 
				$("#form-submit [name='sfCzz'][value=0]").prop('checked','checked'); 
			}else{
				$("#form-submit [name='sfCzz'][value=0]").removeProp("checked").removeAttr("checked"); ; 
				$("#form-submit [name='sfCzz'][value=1]").prop("checked","checked"); 
			}
		});
		// 费用承担部门选择
		bindFybmSelect();
		...
		$("#form-submit [name=ddlb]").on("change", function(e) {
			...
			if($(this).val() == "1" || $(this).val() == "4" || $(this).val() == "13"){
				// 选PI
				...
				$("#form-submit [name=sfCzz]").removeProp("disabled").removeAttr("disabled");
				$("#form-submit [name=fktjmc]").val("");
				$("#form-submit [name=fktj]").val("");

			} else if($(this).val() == "7" || $(this).val() == "14"){ //收费市场物料、工装物料 传制造
				// 选PI
				$("#btn-piSelect").removeProp("disabled").removeAttr("disabled");
				$("#form-submit [name=sfMf][value=0]").prop("checked","checked");
				$("#form-submit [name=zrhf]").prop("disabled","disabled");
				...
				//不传制造且不能选
				$("#form-submit [name=sfCzz][value=0]").prop("checked","checked");
				$("#form-submit [name=sfCzz]").prop("disabled","disabled");
				$("#form-submit [name=fktjmc]").val("");
				$("#form-submit [name=fktj]").val("");
			} else if($(this).val() == "6"){ //免费市场物料 传制造
				// 手动录入
			...
			//初始明细表单价不能编辑，免费定单时，设定明细表单价可以编辑
			var value = $("input[name='sfMf']:checked").val();
			if(value == 1){
				$(grid_wlxx_selector).find("[name='dj']").attr({"disabled":false});
			}
		});
		// 导出按钮
	...
	function bindGridWlxx(){
		$(grid_wlxx_selector).bindTable({
			...
			colModel: [
						...
						{name:'wlms', index:'wlms', label:'物料描述', width:80,editoptions:{disabled:true}},
					    ...
					    {name:'dj', index:'dj', label:'单价（含税）', align:'right',
							 formatter:'number',formatoptions:{thousandsSeparator: ',', decimalPlaces: 6},width:80,editoptions:{disabled:true,dataInit: initDj}},
	...
	function save(){
		...
		//只有客诉专员能够下供应商免费提供备损和 技术服务部客诉备损
		if(param.sfKszy == 0 &&  (param.ddlb==3 || param.ddlb==11)){
			swal("", "请重新选择订单类别，只有客诉专员才能下供应商免费提供备损和 技术服务部客诉备损!", "warning");
			return  ;
		}
		if(param.fktj=="9999" && param.sfMf == 0){
			swal("", "免费备损订单付款条件应该选FREE OF CHARGE!", "warning");
			return  ;
		}
