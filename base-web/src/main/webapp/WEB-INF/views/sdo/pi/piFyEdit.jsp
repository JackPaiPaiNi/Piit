...
<head>
	...
	<%
		...
		/* String sfDyhp = request.getParameter("sfDyhp"); */
	%>
<body>
			...
			<form id="form-submit">
				...
				<div class="row" style="margin: 0px 8px;">
					...
					<div class="col-xs-6 col-sm-3">
						<div class="input-group input-group-sm">
							...
							<select role="select" name="pilx" size="2" class="form-control skyselect">
								 ${fns:loadDictOption('PILX_FY')}
							</select>
						</div>
					</div>
					<div class="col-xs-6 col-sm-3">
				...
				<div class="tab-content">
						...
						<div class="row" style="margin: 0px;">
							...
							<div class="col-xs-6 col-sm-6">
								<div class="input-group-sm">
									<input type="text" name="pimdbz" placeholder="Purpose Remark" class="form-control skydisabled" disabled="disabled"/>
								</div>
							</div>
						</div>
						<div class="space-4"></div>
					...
					<div class="tab-pane fade" id="cpxx-tab">
				    	...
				    	<div class="space-4"></div>
				    	<div class="row drbtn" style="margin: 0px 8px;display:none;">
							<div class="col-xs-6 col-sm-6">
  								...
  								<button id="btn-upload" type="button" class="btn btn-sm">
  									<i class="icon-upload-alt icon-on-right bigger-110"></i>导入
  								</button>
							</div>
						</div>
						<div class="space-4"></div>
				    ...
				    <div class="space-4"></div>
				    <c:import url="/pub/showLog/piFy" charEncoding="UTF-8" />
				</div>
			</form>
			<!-- PAGE CONTENT ENDS -->
		</div><!-- /.col -->
...
<script type="text/javascript">
	...
	var zhSffz = false ; 
	// 权限判断
	<shiro:hasPermission name="pi:piFy:edit">isEdit=true;</shiro:hasPermission>
	// 变量定义结束 
	// 加载开始
	$(function($){
		// select2控件绑定
		...
		// PI类型	
		$('#form-submit [name=pilx]').on('change',function(e){
			if($(this).val() == 7){
				$('#form-submit [name=zhfs]').removeProp('disabled').removeAttr("disabled");
				$('#form-submit [name=pimd]').removeProp('disabled').removeAttr("disabled");
				
			}else{
				$('#form-submit [name=zhfs]').select2("val","");
				$('#form-submit [name=pimd]').select2("val","");
				$('#form-submit [name=zhfs]').prop('disabled','disabled');
				$('#form-submit [name=pimd]').prop('disabled','disabled');
				
			}
			if($(this).val() == 8){
				//明细表隐藏列
				$(grid_selector).showCol('mxlx');
				$(grid_selector).showCol('ckddh');
				$(grid_selector).showCol('jixin');
				$(grid_selector).showCol('jixing');
				$(grid_selector).hideCol('model');
				$(grid_selector).hideCol('khxh');
				$(yxd_grid_selector).showCol('mxlx');
				$(yxd_grid_selector).showCol('ckddh');
				$(yxd_grid_selector).showCol('jixin');
				$(yxd_grid_selector).showCol('jixing');
				$(yxd_grid_selector).hideCol('model');
				$(yxd_grid_selector).hideCol('khxh');
				$('.drbtn').show();
			}else{
				//明细表隐藏列
				$(grid_selector).hideCol('mxlx');
				$(grid_selector).hideCol('ckddh');
				$(grid_selector).hideCol('jixin');
				$(grid_selector).hideCol('jixing');
				$(grid_selector).showCol('model');
				$(grid_selector).showCol('khxh');
				$(yxd_grid_selector).hideCol('mxlx');
				$(yxd_grid_selector).hideCol('ckddh');
				$(yxd_grid_selector).hideCol('jixin');
				$(yxd_grid_selector).hideCol('jixing');
				$(yxd_grid_selector).showCol('model');
				$(yxd_grid_selector).showCol('khxh');
				$('.drbtn').hide();
			}
		});
		// 参考PI号选择
		...
		$("#btn-download").click(function(){
			window.location.href=encodeURI("<%=request.getContextPath()+"/template/piFyItem.xlsx"%>");
	    }); 
		// 导入按钮
		upload();
		// 附件
	...
	function bindPiid(){
		$.tableDialogPage({
			...
			url:"<c:url value='/pi/piFy/search'/>"+ "?sfDyhpi=2&zt=5" 
		},{
			callback:function(rows){
				if(rows){
					...
					// 根据参考订单号初始化数据
					$.bindAjax({
						url : "<c:url value='/pi/piFy/findById'/>",
						data : {id : rows.id},
						action : "search"
					},function(response){
						// 型号。付费备损加载
	...
	function bindGrid(){
		...
		$(yxd_grid_selector).bindTable({
			...
			colModel: [
				...
				{name:'mxlx', index:'mxlx', label:'Type', width:100, hidden: true, editable:true, edittype:"select",formatter: "select",
					editoptions:{value:"${fns:loadDictEditOption('WLLX')}",dataInit: initSelect2}},
				{name:'wlbh', index:'wlbh', label:'Material', width:150, editoptions : {dataInit : InitWL}},
				{name:'model', index:'model', label:'Model', width:150, editoptions:{disabled:true}},
				{name:'khxh', index:'khxh', label:'CustModel', width:150},
				{name:'khxhms', index:'khxhms', label:'Description', edittype:"textarea",width:150},
				{name:'sl', index:'sl', label:'Quantity', align:'right', width:80,
				 formatter:'integer',formatoptions:{thousandsSeparator: ','},editrules:{integer:true},editoptions:{dataInit: initSl}},
				 <shiro:hasPermission name="pi:piFy:price"> 
				 {name:'dj', index:'dj', label:'Unit Price', align:'right', width:80,
				...
				{name:'mfsl', index:'mfsl', label:'免费数量', align:'right', width:80, formatter:'integer',formatoptions:{thousandsSeparator: ','},
							 editrules:{integer:true}},
			 	{name:'ckddh', index:'ckddh', label:'Refer Order', width:120,hidden: true,editoptions : {dataInit : InitDDH}},
				{name:'jixin', index:'jixin', label:'Mechanism', width:80,hidden: true,editoptions:{disabled:true}},
				{name:'jixing', index:'jixing', label:'Model No', width:80,hidden: true,editoptions:{disabled:true}},
		...
		$(grid_selector).bindTable({
			...
			colModel: [
				...
				{name:'dj', index:'dj', label:'Unit Price',align:'right', width:80,
						 formatter:'number',formatoptions:{thousandsSeparator: ',', decimalPlaces: 6},editrules:{number:true},editoptions:{dataInit: initDj}},
				{name:'je', index:'je', label:'Total Amount',align:'right',
		...
		$(elem).parent("span").append('<i class="icon-search green"></i>').click(function() {
			$.tableDialogPage({
				...
				colModel:[{name:'id', index:'id', label:'ID',hidden:true},
						  ...
						  {name:'wldw', index:'wldw', label:'单位', width:40}],
	...
	function InitDDH(elem) {
		...
		$(elem).parent("span").append('<i class="icon-search green"></i>').click(function() {
			$.tableDialogPage({
				title:"订单选择",
				searchCond:[{"name":"参考订单号","value":"ddid"},
				            {"name":"客户名称","value":"khmc"}],
				colModel:[{name:'id', index:'id', label:'ID',hidden:true},
				          {name:'ddid', index:'ddid', label:'参考订单号'},
				          {name:'gs', index:'gs', label:'公司'},
				          {name:'khmc', index:'khmc', label:'客户名称'},
				          {name:'wsxh', index:'wsxh', label:'机型'},
				          {name:'jixin', index:'jixin', label:'机芯'}],
			url: "<c:url value='/order/orderProduct/search'/>?zt=5&sfCk=1"
			},{
				callback:function(rows){
					if(rows){
		...
		if(id != "null"){
			$.bindAjax({
				...
				/* if(response.sfBg == 1){
					...
					//$('#form-submit [name=bgbz]').removeAttr('disabled');
				} */
				// 型号。付费备损加载
				if(response.piItemList){
					var data = response.piItemList;
	...
	function save(){
		...
		$.each(data, function(i,row){ 
			if(row.wlbh.length == 0 || row.wlbh == ""){
				message += "第"+(i+1)+"行，物料编号不能为空.";
    			flag = false;
			}
		}); 
		if(flag == false){
		...
		$("body").bindSweetAlert({
			...
			callback:function(){
				$.bindAjax({
					url:"<c:url value='/pi/piFy/edit'/>",
					data:param,
					action:"save"
				},function(response){
					$("#form-submit").addDisabled();
	...
	function submit(){
		...
		$("body").bindSweetAlert({
			...
			callback:function(){
				$.bindAjax({
					url:"<c:url value='/pi/piFy/submit'/>",
					data:param,
					action:"save"
				},function(response){
					$('#form-submit [name=token]').val(response.token);
					window.history.back(-1);
				}); 
			}
		});
	...
	function upload(){
		$('#btn-upload').click(function(){
			$.importDialog({
				...
				url:"<c:url value='/pi/piFy/upload'/>"
			},{
				callback:function(results){
					var rows = eval("(" + results + ")");
					$.each(rows.items,function(index,row){
						row.je  =  row.dj*row.sl ;
					}); 
	...
	function sumGrid(){
		var je;
		$(grid_selector).footerData("set",{"wlbh":"合计"});
		var rowNum = $(grid_selector).jqGrid('getGridParam','records');
		if(rowNum >0 ){
