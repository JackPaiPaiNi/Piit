					...
					<div class="widget-main">
						<form id="form-search" class="form-search">
							<div class="row">
								...
								<input type ="hidden" name="pilx" id="pilx" value="1">
								<div class="col-xs-6 col-sm-3">
									<div class="input-group input-group-sm">
										<input type="hidden" name="sfDyhpi" value="0">
							...
							<div class="row">
								...
								<shiro:hasPermission name="pi:pi:smoApprove">
									<div class="col-xs-6 col-sm-3">
										<div class="input-group input-group-sm">
											<label class="input-group-addon">SMO审核状态（LG客户）&nbsp;&nbsp;</label>
											<select role="select" name="sfSmosh" size="1" class="form-control skyselect">
												${fns:loadDictOption('SMOSHZT')}
											</select>
										</div>
									</div>
								</shiro:hasPermission>
...
<script type="text/javascript">
	...
	var editurl = "<c:url value='/pi/pi/editPage'/>";
	var viewurl = "<c:url value='/pi/pi/viewPage'/>";
	var delurl = "<c:url value='/pi/pi/delete'/>";
	var searchurl= "<c:url value='/pi/pi/search'/>";
	var expurl = "<c:url value='/pi/pi/export'/>";
	var changeurl = "<c:url value='/pi/pi/change'/>";
	var bfchangeurl = "<c:url value='/pi/pi/bfchange'/>";
	var cancelurl = "<c:url value='/pi/pi/cancel'/>";
	var getbackurl = "<c:url value='/pi/pi/getback'/>";
	var isEdit = false;
	...
	$(function($) {
		...
		$(grid_selector).bindTable({
			...
			colModel: [
				...
				{name:'gsbm', index:'gsbm', label:'公司', width:50},
				...
				<shiro:hasPermission name="pi:pi:price"> 
			     	{name:'zje', index:'zje', label:'总金额', align:'right',width:100,
					 formatter:'number',formatoptions:{thousandsSeparator: ','}},
				</shiro:hasPermission>
				{name:'bz', index:'bz', label:'币种', width:40},
				{name:'zdrmc', index:'zdrmc', label:'制单人', width:70},
	            {name:'sfSmosh', index:'sfSmosh', label:'SMO审核状态（LG客户）', width:130, formatter:smoshCellShow},
				{name:'sfCh', index:'sfCh', label:'是否撤回', hidden:true}
			]
		},{
			add:isEdit,
			edit:isEdit,
		...
		<shiro:hasPermission name="pi:pi:edit">
		$(grid_selector).navButtonAdd(pager_selector,{
			caption:"撤回", 
			buttonicon:"fa-pencil-square grey", 
			onClickButton: function(){
		...
		$(grid_selector).navButtonAdd(pager_selector,{
			...
			onClickButton: function(){
				...
				if(id==null || id==""){
					...
					var rowData = jQuery(grid_selector).jqGrid('getRowData',id);
					if(rowData.zt != 5 || rowData.sfSmosh != "未审核"){
						swal("","请选择一行已生效且未进行SMO审核的数据！","warning");
						return;
					}
					$.bindAjax({
						url:"<c:url value='/pi/pi/smoApprove'/>",
						data:{id:id, sjc:rowData.sjc},
						action:"save"
					},function(response){
						$(grid_selector).trigger("reloadGrid");
