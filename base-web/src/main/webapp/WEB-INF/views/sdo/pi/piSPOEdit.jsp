		...
		<div class="col-xs-12">
			...
			<form id="form-submit">
				<div class="row" style="margin: 0px 8px;">
					...
					<div class="col-xs-6 col-sm-6">
						<button id="btn-save" type="button" class="btn btn-success btn-sm">
							<i class="icon-save  btn-success icon-on-right bigger-110"></i>
							保存
						</button>
						&nbsp;
				...
				<div class="col-xs-6 col-sm-3">
					...
					<div class="col-xs-6 col-sm-3">
						<div class="input-group input-group-sm">
							<label class="input-group-addon">备损PI类别&nbsp;&nbsp;</label>
							<select role="select" name="pilb" size="1" class="form-control skyselect">
								 ${fns:loadDictOption('PILB')}
							</select>
						</div>
					</div>
				</div>
				...
				<div class="tab-content">
				    ...
				    <div class="space-4"></div>
				    <c:import url="/pub/showLog/piSpo" charEncoding="UTF-8" />
				</div>
			</form>
			<!-- PAGE CONTENT ENDS -->
		</div><!-- /.col -->
...
<script type="text/javascript">
	...
	var zhSffz = false ;
	// 权限判断
	<shiro:hasPermission name="pi:piSPO:edit">isEdit=true;</shiro:hasPermission>
	// 变量定义结束 
	// 加载开始
	$(function($){
		// select2控件绑定
		...
		$("#btn-download").click(function(){
			window.location.href=encodeURI("<%=request.getContextPath()+"/template/PiPSOItem.xlsx"%>");
	    }); 
		// 导入按钮
		upload();
		//绑定银行信息加载时间
		...
		$("#form-submit [name=bz],#form-submit [name=fktj],#form-submit [name=gsbm]").on('change',function(e) {
			changeYhxx();
		});
	});
	/**************************************************function开始区域************************************************/
...
// 						$("#form-submit [name=syrdz]").val(e.added.gsywdz);
	...
	function bindKhbm(){
		$.tableDialogPage({
			...
			colModel:[{name:'id', index:'id', label:'ID',hidden:true}, 
					  ...
					  {name:'khlx', index:'khlx', label:'客户类型', width:80},
					  {name:'fktjms', index:'fktjms', label:'付款条件', width:80}],
	...
	function bindPiid(){
		$.tableDialogPage({
			...
			url:"<c:url value='/pi/pi/search'/>"+ "?sfDyhpi=0&pilx=2&zt=5" 
		},{
			callback:function(rows){
				if(rows){
					...
					// 根据参考订单号初始化数据
					$.bindAjax({
						url : "<c:url value='/pi/piSPO/findById'/>",
						data : {id : rows.id},
						action : "search"
					},function(response){
						// 型号。付费备损加载
	...
	function bindGrid(){	
		...
		$(yxd_grid_selector).bindTable({
			...
			colModel: [
				...
				{name:'mxlx', index:'mxlx', label:'Type', width:100, editable:true, edittype:"select",formatter: "select",
					editoptions:{value:"${fns:loadDictEditOption('WLLX')}",dataInit: initSelect2}},
				{name:'wlbh', index:'wlbh', label:'Meterial No', width:150, editoptions : {dataInit : InitWL}},
				{name:'khxhms', index:'khxhms', label:'Description', edittype:"textarea", width:150},
				{name:'dw', index:'dw', label:'Unit', width:80,editoptions:{disabled:true}},
			    ...
			    <shiro:hasPermission name="pi:piSPO:price"> 	
				...
				,formatter:function(cellvalue, options, row){return (row.dj*row.sl).toFixed(6);}},
				</shiro:hasPermission>
				{name:'ckddh', index:'ckddh', label:'Refer Order', width:120,editoptions : {dataInit : InitDDH}},
				{name:'jixin', index:'jixin', label:'Mechanism', width:80,editoptions:{disabled:true}},
				{name:'jixing', index:'jixing', label:'Model No', width:80,editoptions:{disabled:true}},
		...
		$(grid_selector).navButtonAdd(pager_selector,{
			caption:"清空", 
			buttonicon:"fa-pencil-square red", 
			onClickButton: function(){
				$(grid_selector).jqGrid("clearGridData");
				$(grid_selector).footerData("set",{"sl":0, "je":0}); 
			},
			position:"last"
		});
	}
			...
			url: "<c:url value='/pub/widget/findMaterial'/>"
				...
				callback:function(rows){
					if(rows){
						...
						$(elem).closest("tr.jqgrow").find("textarea[name='khxhms']").val(rows.wlms);
						$(elem).val(rows.wlbh);
					}else{
						$(elem).closest("tr.jqgrow").find("input[name='dw']").val("");
						$(elem).closest("tr.jqgrow").find("input[name='dj']").val("");
						$(elem).closest("tr.jqgrow").find("input[name='dj']").removeProp("disabled").removeAttr("disabled");
						$(elem).closest("tr.jqgrow").find("textarea[name='khxhms']").val("");
						$(elem).val("");
					}
	...
	function bindFormData(id){	
		...
		if(id != "null"){
			...
			$("#form-submit [name=pilx]").val("2");//PI类型:备损PI
			$("#form-submit [name=oper]").val("add");//权限:add
			$("#form-submit [name=zt]").val("1");//PI状态:草稿
			$("#form-submit [name=zhfs]").select2("val","1");//走货方式：CKD
			$("#form-submit [name=bz]").select2("val","USD");//币种:USD
			$("#form-submit [name=pimd]").select2("val","3");;//PI目的:Order
			$("#form-submit [name=syr]").val("Skyworth M C O Company Ltd.");//受益人
			$("#form-submit [name=syrdz]").val("EM MACAU, AVENIDA DA PRAIA GRANDE NO 619, EDF COMERCIAL SI TOI K7");//受益人地址
	...
	function submit(){
		...
		$("body").bindSweetAlert({
			...
			callback:function(){
				$.bindAjax({
					url:"<c:url value='/pi/piSPO/submit'/>",
					data:param,
					action:"save"
				},function(response){
	...
	function upload(){
		$('#btn-upload').click(function(){
			$.importDialog({
				...
				url:"<c:url value='/pi/piSPO/upload'/>"
			},{
	...
	function initDj(elem){
		...
		if(xszz == '0105' && mxlx != '4'){
			if($(elem).closest("tr.jqgrow").find("[name='wlbh']").val().length > 0 && $(elem).closest("tr.jqgrow").find("[name='dj']").val().length>0){
				$(elem).closest("tr.jqgrow").find("[name='dj']").prop('disabled','disabled');
			}else{
				$(elem).closest("tr.jqgrow").find("[name='dj']").removeProp("disabled").removeAttr("disabled");
			} 
