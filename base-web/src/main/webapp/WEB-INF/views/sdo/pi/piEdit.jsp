		...
		<div class="col-xs-12">
			...
			<form id="form-submit">
				...
				<div class="row" style="margin: 0px 8px;">
					...
					<div class="col-xs-6 col-sm-3">
						<div class="input-group input-group-sm">
							<label class="input-group-addon">是否分公司特价&nbsp;&nbsp;</label>
							<div class="form-control">
								<label class="radio-inline">
									<input type="radio" class="skyradio skydisabled" disabled="disabled" name="sfFgstj" value="1" onclick="show_tj();">是
								</label>
								<label class="radio-inline">
									<input type="radio" class="skyradio skydisabled" disabled="disabled" name="sfFgstj" value="0" checked="checked" onclick="hidden_tj();">否
								</label>
							</div>
						</div>
					</div>
				...
				<div class="tab-content">
						...
						<div class="row" style="margin: 0px;">
							...
							<div class="col-xs-6 col-sm-3">
								<div class="input-group input-group-sm">
									...
									<select id = "pimd" role="select" name="pimd" size="1" class="form-control skyselect " >
				    ...
				    <div class="space-4"></div>
				    <c:import url="/pub/showLog/piTv" charEncoding="UTF-8" />
				</div>
			</form>
			<!-- PAGE CONTENT ENDS -->
		</div><!-- /.col -->
...
<script type="text/javascript">
	...
	var zhSffz = false ; 
	// 权限判断
	<shiro:hasPermission name="pi:pi:edit">isEdit=true;</shiro:hasPermission>
	// 变量定义结束 
	// 加载开始
	$(function($){
		var data = {};
	...
	function changeYhxx(){
		...
		$.post("<c:url value='/pub/select2/companyBank'/>",{gsbm:gsbm,bz:bz},
				function(result){
					var data = $.map(result, function (obj) {
						obj.id = obj.ywmc;
					    obj.text = obj.text || obj.ywmc;
					    return obj;
					});
					if(sfLc =="0"){
						$("#form-submit [name=skyhmc]").addClass("skyselect").select2({data:data}).on("change",function(e){
							if(e.added){
								$("#form-submit [name=skyhmc]").val(e.added.ywmc).trigger("change");
						...
						$("#form-submit [name=lcskyhmc]").addClass("skyselect").select2({data:data}).on("change",function(e){
							if(e.added){
								$("#form-submit [name=lcskyhmc]").val(e.added.ywmc).trigger("change");
	...
	function bindKhbm(){
		$.tableDialogPage({
			...
			callback:function(rows){
				if(rows){
					$("input[name=xyedye]").val(rows.xyedye);
					...
					$("input[name=fktj]").select2("val",rows.fktj).trigger("change");
					if(rows.yxhts == 0){
						swal("","该客户尚未签订有效合同，请跟进合同的签订", "warning");
					}
				}else{
					$("input[name=xyedye]").val("");
					$("input[name=khbm]").val("");
					$("input[name=khmc]").val("");
					$("input[name=khdz]").val("");
					$("input[name=khdh]").val("");
					...
					$("input[name=khpp]").val("");
					$("input[name=fktj]").select2("val","").trigger("change");
				}
			}
		});
	}
	...
	function selectXsyInit(){
		$("#form-submit [name=xsymc]").bindTreeTableDialog({
			...
			callback:function(record){
				...
				$("input[name=khlxr]").val("");
				$("input[name=fktj]").select2("val","").trigger("change");
				if(record.xszz == '0105'){//品牌分公司客户
					$('#form-submit [name=sfFgstj]').removeProp('disabled').removeAttr("disabled");
					show_tj();
				}else{
					$("#form-submit [name=sfFgstj][value=0]").prop("checked","checked");
					$('#form-submit [name=sfFgstj]').prop('disabled','disabled');
					hidden_tj();
				}
			}
		});
	...
	function bindPiid(){
		$.tableDialogPage({
			...
			colModel:[{name:'id', index:'id', label:'ID',hidden:true},
			          ...
			          {name:'khlx', index:'khlx', label:'客户类型', hidden:true},
					  {name:'khlx', index:'khlx', label:'客户类型'},
			          {name:'bbh', index:'bbh', label:'参考PI版本号'}],
			url:"<c:url value='/pi/pi/search'/>"+ "?sfDyhpi=0&pilx=1&zt=5" 
		},{
			callback:function(rows){
				if(rows){
					//2017-10-23复制PI
					...
					// 根据参考订单号初始化数据
					$.bindAjax({
						url : "<c:url value='/pi/pi/findById'/>",
						data : {id : rows.id},
						action : "search"
					},function(response){
						// 型号。付费备损加载
				...
				if(rows.skyhzh){
					$("#form-submit [name=skyhdm]").trigger("change", 0);
					$("#form-submit [name=skyhzh]").val(rows.skyhzh).trigger("change");
				}
				if(rows.lcskyhzh){
					$("#form-submit [name=lcskyhdm]").trigger("change", 0);
					$("#form-submit [name=lcskyhzh]").val(rows.lcskyhzh).trigger("change");
				}
				if(rows.fktj){
					$("#form-submit [name=fktj]").val(rows.fktj).trigger("change");
				}
			}
		});
	}
	...
	function bindGrid(){	
		...
		$(yxd_grid_selector).bindTable({
			...
			colModel: [
				...
				{name:'mxlx', index:'mxlx', label:'行类型', width:80, editable:true, edittype:"select",formatter: "select",
					editoptions:{value:"${fns:loadDictEditOption('PIMXLX')}",dataInit: initMxlx}, editrules:{required:true}},
				{name:'pid', index:'pid', label:'PID', width:80,editoptions : {dataInit : InitPID}},
				{name:'jixing', index:'jxing', label:'Model', width:82,editable:true, edittype:"select",formatter: "select",
					editoptions:{value:"${fns:loadModelOption('1','1')}",dataInit: initSelect2}, editrules:{required:true}},
				{name:'jixin', index:'jxin', label:'Chassis', width:82,editable:true, edittype:"select",formatter: "select",
				...
				{name:'ks', index:'ks', label:'款式', width:42, editable:true, edittype:"select",formatter: "select",
					editoptions:{value:"${fns:loadDictEditOption('KS')}",dataInit: initSelect2}},
				{name:'sl', index:'sl', label:'Quantity',align:'right', width:50, formatter:'integer',formatoptions:{thousandsSeparator: ','} ,
					editrules:{integer:true, required:true} ,editoptions:{dataInit: initSl}},
				{name:'tj', index:'tj', label:'特价',align:'right', width:50,formatter:'number',editrules:{number:true},formatoptions:{thousandsSeparator: ',',decimalPlaces:4},
					editoptions:{dataInit: initTj}},
				<shiro:hasPermission name="pi:pi:price"> 		
				{name:'dj', index:'dj', label:'Unit Price',align:'right', width:50,formatter:'number',editrules:{number:true, required:true},
					formatoptions:{thousandsSeparator: ',',decimalPlaces:6}, editoptions:{dataInit: initDj}},
				{name:'je', index:'je', label:'Total Amount', align:'right',width:60,formatter:'number',formatoptions:{thousandsSeparator: ',',decimalPlaces:6},
		...
		$(grid_selector).bindTable({
			...
			editComplete: function(lastEdit, rowData){
				...
				if($('#form-submit [name=sfFgstj]:checked').val()=="0"){
					var xszz = $("#form-submit [name=xszz]").val();//销售员所属部门编码
					if((xszz == '0105') && (rowData.mxlx=='1' || rowData.mxlx=='2' || rowData.mxlx=='3'|| rowData.mxlx=='5')){
						var dj="";
						...
						if(dj != "" && dj != "0"){
							...
							if(rowData.sl!='' && rowData.sl!=null){
								$(grid_selector).jqGrid('setRowData', lastEdit, {je:parseFloat(rowData.sl*dj).toFixed(6)}); 
							}
						}else{
							swal("", "该型号没有有效价格，请先通知成本价格组维护价格!", "warning");
							// 清空单价
							$(grid_selector).jqGrid('setRowData', lastEdit, {dj:0}); 
						}
					}
				}
				sumGrid();
		...
		$(grid_selector).navButtonAdd(pager_selector,{
			...
			onClickButton: function(){
				...
				if(selr==null||selr==""){
					...
					$(grid_selector).jqGrid("addRowData", null, rowData, "last"); 
				}
			},
			position:"last"
		});
	...
	function bindFormData(id){
		...
		if(id != "null"){
			$.bindAjax({
				...
				if(response.skyhzh){
					$("#form-submit [name=skyhdm]").trigger("change", 0);
					$("#form-submit [name=skyhzh]").val(response.skyhzh).trigger("change");
				}
				if(response.lcskyhzh){
					$("#form-submit [name=lcskyhdm]").trigger("change", 0);
					$("#form-submit [name=lcskyhzh]").val(response.lcskyhzh).trigger("change");
				}
				if(response.fktj){
					$("#form-submit [name=fktj]").val(response.fktj).trigger("change");
				}
				$(grid_selector).jqGrid("clearGridData");
	...
	// 隐藏明细表特价字段
	function hidden_tj(){
		$(grid_selector).hideCol("tj");
		$(grid_selector).showCol("dj");
	}
	// 显示明细表特价字段
	function show_tj(){
		if($('#form-submit [name=sfFgstj]:checked').val()=="1"){
			$(grid_selector).showCol("tj");
			$(grid_selector).hideCol("dj");
		}
	}
	// 计算合计行
	function sumGrid(){
	...
	function initDj(elem){
		...
		/* 新增 初始化时根据行类型判断单价是否可编辑 -------20161110赵明-------开始*/
		var mxlx = $(elem).closest("tr.jqgrow").find("[name='mxlx']").val();
		var xszz=$("#form-submit [name=xszz]").val();//销售员所属部门编码
		if(xszz.length>=4 && (xszz.substring(0,4)=='0105' ) && (mxlx=='1' || mxlx=='2' || mxlx=='3'|| mxlx=='5')){
			$(elem).closest("tr.jqgrow").find("[name='dj']").prop('disabled','disabled');
		}else{
			$(elem).closest("tr.jqgrow").find("[name='dj']").removeProp("disabled").removeAttr("disabled");
		} 
		/* 新增 初始化时根据行类型判断单价是否可编辑 -------------------------结束*/
	}
	/* 新增 初始化特价 -------20161110赵明-------开始*/
	function initTj(elem){
		$(elem).on("blur", function(){
			var sl = $(elem).closest("tr.jqgrow").find("[name='sl']").val();
			var tj = $(elem).val();
			var je = "";
	...
	//初始化明细类型
	function initMxlx(elem){
		$(elem).addClass("skyselect").select2({width:"100%"}).on("change", function(){
			var mxlx = $(elem).val();
			var xszz=$("#form-submit [name=xszz]").val();//销售员所属部门编码
			if(xszz.length>=4 && (xszz.substring(0,4)=='0105') && (mxlx=='1' || mxlx=='2' || mxlx=='3'|| mxlx=='5')){
				//$(elem).closest("tr.jqgrow").find("[name='dj']").attr({"disabled":true});
				$(elem).closest("tr.jqgrow").find("[name='dj']").prop('disabled','disabled');
			}else{
				//$(elem).closest("tr.jqgrow").find("[name='dj']").attr({"disabled":false});
				$(elem).closest("tr.jqgrow").find("[name='dj']").removeProp("disabled").removeAttr("disabled");
			} 
		});
	} 
