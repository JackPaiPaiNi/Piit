		...
		<div class="col-xs-12">
			...
			<form id="form-submit">
				...
				<div class="row" style="margin: 0px 8px;">
					...
					<div class="col-xs-6 col-sm-3">
						<div class="input-group input-group-sm">
							...
							<select role="select" name="pilx" size="1" class="form-control skyselect">
								 ${fns:loadDictOption('PILX_DYH')}
							</select>
						</div>
					</div>
					<div class="col-xs-6 col-sm-3">
						...
						<div class="input-group input-group-sm">
							...
							<div class="form-control">
								<label class="radio-inline">
									<input type="radio" class="skyradio" name="sfBd" value="0" checked="checked">否
								</label>
								<label class="radio-inline">
									<input type="radio" class="skyradio" name="sfBd" value="1">是
								</label>
							</div>
						</div>
					</div>
				...
				<div class="row" style="margin: 0px 8px;">
					<!-- <div class="col-xs-6 col-sm-6">
						<div class="input-group input-group-sm">
							...
							<label class="input-group-addon">多元化采购供应商PI附件&nbsp;&nbsp;</label>
							<span class="input-group-addon">
							<input id="fj" type="file" class="form-control"></span>
							<div id="dyhcggyspi" class="form-control"></div>
				...
				<div class="tab-content">
				    ...
				    <div class="space-4"></div>
				    <c:import url="/pub/showLog/piDiversity" charEncoding="UTF-8" />
				</div>
			</form>
			<!-- PAGE CONTENT ENDS -->
		</div><!-- /.col -->
...
<script type="text/javascript">
	...
	function bindPiid(){
		$.tableDialogPage({
			...
			url:"<c:url value='/pi/piDiversity/search'/>"+ "?sfDyhpi=1&zt=5" 
		},{
			callback:function(rows){
				if(rows){
					...
					// 根据参考订单号初始化数据
					$.bindAjax({
						url : "<c:url value='/pi/piDiversity/findById'/>",
						data : {id : rows.id},
						action : "search"
					},function(response){
						// 型号。付费备损加载
	...
	function bindFormData(id){	
		...
		if(id != "null"){
			$.bindAjax({
				...
				$(grid_selector).jqGrid("clearGridData");
				if(sfDyhp == "1"){
					$("#form-submit [name=sfBd]").prop("disabled","disabled").addClass("skydisabled");
				}else{
					$("#form-submit [name=sfBd]").removeProp('disabled').removeAttr('disabled');
				}
			...
			if(sfDyhp == "1"){
				$("#form-submit [name=pilx]").select2("val", "5");//PI类型:多元化屏PI
				$("#form-submit [name=sfBd]").prop("disabled","disabled").addClass("skydisabled");
			}else {
				$("#form-submit [name=pilx]").select2("val", "4");//PI类型:多元化PI
				$("#form-submit [name=sfBd]").removeProp('disabled').removeAttr('disabled');
			}
			$("#form-submit [name=oper]").val("add");//权限:add
	...
	function save(){
		...
		$("body").bindSweetAlert({
			...
			callback:function(){
				$.bindAjax({
					url:"<c:url value='/pi/piDiversity/edit'/>",
					data:param,
					action:"save"
				},function(response){
	...
	function submit(){
		...
		$("body").bindSweetAlert({
			...
			callback:function(){
				$.bindAjax({
					url:"<c:url value='/pi/piDiversity/submit'/>",
					data:param,
					action:"save"
				},function(response){
