package com.ey.piit.core.redis;
...
import com.ey.piit.core.redis.JedisTemplate.JedisAction;
import com.ey.piit.core.redis.JedisTemplate.JedisActionNoResult;
import com.ey.piit.core.redis.JedisTemplate.PipelineAction;
import com.ey.piit.core.redis.JedisTemplate.PipelineActionNoResult;
...
public class JedisShardedTemplate {
	...
	public JedisTemplate getShard(byte[] key) {
		if (singleTemplate != null) {
			return singleTemplate;
		}
	...
	public Set<byte[]> keys(byte[] keys) {
		JedisTemplate jedisTemplate = getShard(keys);
		return jedisTemplate.keys(keys);
	}
	...
	public void expire(byte[] key, int value) {
		JedisTemplate jedisTemplate = getShard(key);
		jedisTemplate.expire(key, value);
	}
}
