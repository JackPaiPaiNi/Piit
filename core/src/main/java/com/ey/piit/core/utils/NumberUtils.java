...
public class NumberUtils extends org.apache.commons.lang3.math.NumberUtils {
	...
	/**
	 * 阿拉伯数字转换成英文大写
	 * 异常：负数异常、小数点后两位以上异常、非数字异常
	 */
	public static String toEnglish(String x) {
		if (x == null || !x.matches("\\d+\\.?\\d*")) {
			throw new RuntimeException("错误的参数："+x+"，请输入有效的数字");
		}
		if (x.matches("\\d+\\.\\d{3,}")) {
			throw new RuntimeException("错误的参数："+x+"，请精确到小数点后两位");
		}
		...
		if (toInt(rstr) != 0) {
			if (toInt(lstr) == 0) {
				return transTwo(rstr) + " CENTS";
			} else {
				return lm.trim() + " AND " + transTwo(rstr) + " CENTS";// 小数部分存在时转换小数
			}
		} else {
			return lm.trim() + " ONLY";
		}
	...
	private static String parseTeen(String s) {
		String[] a = new String[] { "TEN", "ELEVEN", "TWELEVE", "THIRTEEN", "FOURTEEN", "FIFTEEN", "SIXTEEN",
